...
function initTable(){
	...
	$('#table1').bootstrapTable({
		url: 'staticCashFlowGapStatistics1.json',
		cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
		pagination: false, //是否显示分页（*）
		columns: columns
	});
	$('#table2').bootstrapTable({
		url: 'staticCashFlowGapStatistics2.json',
		cache: false,
		pagination: false,
		columns: columns
	});
	$('#table3').bootstrapTable({
		url: 'staticCashFlowGapStatistics3.json',
		cache: false,
		pagination: false,
		columns: columns
	});
...
function initProgress(){
	var barNode = $('#progressBar')
	barNode.attr("style", "width: 100%")
	barNode.parent().delay(1000).hide(0);
//	barNode.remove()
}
